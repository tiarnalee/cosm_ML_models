...
def decode_seg_map_sequence(label_masks, dataset='pascal'):
    ...
    rgb_masks = torch.from_numpy(np.array(rgb_masks))
    return rgb_masks
...
def decode_segmap(label_mask, dataset, plot=False):
    ...
    elif dataset == 'NUMPY_dataloader1':
            n_classes = 2
    ...
    rgb = np.zeros((label_mask.shape[0], label_mask.shape[1]))
    rgb=r/255.0
    # rgb[:, :, 0] = r / 255.0
    # rgb[:, :, 1] = g / 255.0
    # rgb[:, :, 2] = b / 255.0
